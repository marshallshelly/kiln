...
/// Resolves and loads ES modules off disk, rooted at the app directory.
///
/// rquickjs ships a `FileResolver`, but it resolves through `RelativePath`
/// against the process working directory, so a page anywhere else never
/// resolves. Rooting it ourselves is both correct and the place to enforce
/// that an app cannot import its way out of its own directory — the same rule
/// `LocalFiles` applies to stylesheets and images.
#[derive(Clone)]
struct AppModules {
...
impl AppModules {
    ...
    fn locate(&self, base: &str, name: &str) -> Option<std::path::PathBuf> {
        ...
        let resolved = candidate
            .canonicalize()
            .or_else(|_| candidate.with_extension("js").canonicalize())
            .ok()?;
...
impl rquickjs::loader::Resolver for AppModules {
    fn resolve(
        &mut self,
        _ctx: &Ctx<'_>,
        base: &str,
        ...
        self.locate(base, name)
            .map(|path| path.to_string_lossy().into_owned())
            .ok_or_else(|| rquickjs::Error::new_resolving(base, name))
    }
}

impl rquickjs::loader::Loader for AppModules {
    fn load<'js>(
        &mut self,
        ctx: &Ctx<'js>,
        name: &str,
...
/// Bundlers address their own assets with `new URL("./thing.png",
/// import.meta.url)`, so a module with no `url` sends every asset request to
/// the wrong directory. QuickJS leaves the object empty for the host to fill.
fn set_import_meta(
    module: &rquickjs::Module<'_, rquickjs::module::Declared>,
    name: &str,
) -> rquickjs::Result<()> {
    module.meta()?.set("url", format!("file://{name}"))
}

/// rquickjs reports "Exception generated by QuickJS" and keeps the message on
/// the context, which is how a real stack trace turns into no information at
/// all.
fn describe_exception(ctx: &Ctx<'_>, error: &rquickjs::Error) -> anyhow::Error {
...
impl Script {
    ...
    /// Lets `import "./thing.js"` resolve against the page's own directory.
    pub fn set_module_root(&self, root: &std::path::Path) {
        let modules = AppModules::new(root);
        self.runtime.set_loader(modules.clone(), modules);
    }

    pub fn eval_module(&self, name: &str, source: &str) -> Result<()> {
        let result = self
            .context
            .with(|ctx| {
                let (_, promise) = rquickjs::Module::declare(ctx.clone(), name, source)
                    .and_then(|module| {
                        set_import_meta(&module, name)?;
                        module.eval()
                    })
                    .map_err(|error| describe_exception(&ctx, &error))?;
                promise.finish::<()>().map_err(|error| {
                    // A module that throws surfaces here rather than at declare
                    // time, so the rejection is where the real message lives.
                    describe_exception(&ctx, &error)
                })
            })
